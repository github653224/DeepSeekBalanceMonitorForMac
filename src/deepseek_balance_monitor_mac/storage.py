...
from deepseek_balance_monitor_mac.config import DB_FILE, CONFIG_DIR, log
...
def _sum_consumption_drops(values):
    """Sum all downward topped-balance moves while ignoring top-ups."""
    if not values:
        return 0.0
    total = 0.0
    prev = float(values[0])
...
def get_today_consumption(date_str: str | None = None):
    """Return today's topped-balance consumption as (amount, currency).

    The value is computed from cumulative downward moves in topped balance,
    so balance top-ups during the day do not hide actual usage.
    """
    try:
        if date_str is None:
        ...
        by_currency = {}
        latest_timestamp = {}
        for ts, currency, topped in rows:
            by_currency.setdefault(currency, []).append(topped)
            latest_timestamp[currency] = ts

        source_currency = max(latest_timestamp, key=latest_timestamp.get)
        amount = _sum_consumption_drops(by_currency[source_currency])
        return round(amount, 2), source_currency
    except Exception as e:
        log(f"Failed to compute today's consumption: {e}")
        return None
...
def prune_old_data(retention_days: int):
    ...
    try:
        from deepseek_balance_monitor_mac.config import LOG_FILE
