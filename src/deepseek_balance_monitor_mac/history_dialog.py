...
from deepseek_balance_monitor_mac.config import T
from deepseek_balance_monitor_mac.infra.exchange_rates import get_rate
from deepseek_balance_monitor_mac.storage import export_all_csv, get_consumption_rate, get_history_by_date, get_history_page, get_today_consumption
...
def open_history(app):
    ...
    def _update_rate_label():
        if app.demo_mode:
            ...
            rate_var.set(
                f"{T('today_consumption', lang)}: --\n"
                f"{T('rate_line', lang, rate=app._demo_daily, prefix=prefix, remaining=remaining)}"
            )
            return
        today_line = f"{T('today_consumption', lang)}: --"
        today = get_today_consumption()
        if today:
            today_amount, today_currency = _display_amount(*today)
            today_line = f"{T('today_consumption', lang)}: {today_amount:.2f} {today_currency}"
        cr = get_consumption_rate()
        if cr:
            daily_rate, hours_left, curr = cr
            daily_display, daily_currency = _display_amount(daily_rate, curr)
            ...
            rate_var.set(
                f"{today_line}\n"
                f"{T('avg_consumption', lang)}: {daily_display:.2f} {daily_currency}/{'day' if lang == 'en' else '天'}  |  {prefix} {remaining}"
            )
        else:
            rate_var.set(f"{today_line}\n{T('not_enough_data', lang)}")
...
if __name__ == "__main__":
    from deepseek_balance_monitor_mac.config import load_config

    class _StandaloneHistoryApp:
        def __init__(self):
    ...
    open_history(_StandaloneHistoryApp())
