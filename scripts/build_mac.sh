...
ZIP_NAME="DeepSeek-Balance-Monitor-mac.zip"
...
assets = os.path.join(root, 'src/deepseek_balance_monitor_mac/assets')
FONT    = os.path.join(assets, 'font/ShareTech-Regular.ttf')
OUT_PNG = os.path.join(assets, 'AppIcon.png')
OUT_ICS = os.path.join(assets, 'AppIcon.icns')
...
PYEOF
echo -e " - Icon   : ${GREEN}src/deepseek_balance_monitor_mac/assets/AppIcon.icns${NC}"

echo -e "\n${BLUE}[2/4] Checking PyInstaller...${NC}"
if ! "$PYTHON" -m PyInstaller --version &>/dev/null; then
  echo -e "${RED}Error: PyInstaller not found in $PYTHON environment.${NC}"
  echo -e "Install with: uv pip install pyinstaller"
  exit 1
fi
...
rm -f "$ZIP_PATH"
echo -e " - Archive: ${YELLOW}Creating distributable zip...${NC}"
ditto -c -k --sequesterRsrc --keepParent "$APP_BUNDLE" "$ZIP_PATH"
ZIP_SIZE=$(du -sh "$ZIP_PATH" | cut -f1 | xargs)
...
echo -e "${GREEN}============================================${NC}"
echo -e " App  : ${GREEN}${APP_SIZE}${NC} → $APP_BUNDLE"
echo -e " Zip  : ${GREEN}${ZIP_SIZE}${NC} → $ZIP_PATH"
echo -e " SHA  : ${GREEN}${SHA256}${NC}"
